...
pub fn parse_app<R: Read>(reader: &mut R, marker: Marker) -> Result<Option<AppData>> {
    ...
    match marker {
        ...
        APP(2) => {
            ...
            println!(
                "{:?}",
                ISO_METADATA_HEADER.iter().map(|&x| x).collect::<Vec<u8>>()
            );
            if length > ISO_METADATA_HEADER.len() + 2 {
                let mut buffer = [0u8; ISO_METADATA_HEADER.len()];
                reader.read_exact(&mut buffer)?;
                bytes_read = buffer.len();

                buffer[ISO_METADATA_HEADER.len() - 1] = 0;

                if let Ok(string) = std::str::from_utf8(&buffer[0..ISO_METADATA_HEADER.len()]) {
                    println!("{}", string);
                } else {
                ...
                // http://www.color.org/ICC_Minor_Revision_for_Web.pdf
                // B.4 Embedding ICC profiles in JFIF files
                if buffer[0..ISO_METADATA_HEADER.len()] == *ISO_METADATA_HEADER {
                    let mut data = vec![0; length - bytes_read];
                    reader.read_exact(&mut data)?;
                    bytes_read += data.len();
                    result = Some(AppData::GainMap(IccChunk {
                        seq_no: buffer[ISO_METADATA_HEADER.len()],
                        num_markers: buffer[ISO_METADATA_HEADER.len() + 1],
                        data,
                    }));
                }
            }
