...
impl<R: Read> Decoder<R> {
    ...
    /// Returns the embeded gain map profile if the image contains one.
    pub fn gain_map(&self) -> Option<Vec<u8>> {
        let mut marker_present: [Option<&IccChunk>; 256] = [None; 256];
        let num_markers = self.gain_map.len();
        if num_markers == 0 || num_markers >= 255 {
            return None;
        }
        // check the validity of the markers
        for chunk in &self.gain_map {
            if usize::from(chunk.num_markers) != num_markers {
                // all the lengths must match
                return None;
            }
                ...
                Marker::APP(..) => {
                    if let Some(data) = parse_app(&mut self.reader, marker)? {
                        match data {
                            ...
                            AppData::GainMap(gain_map) => self.gain_map.push(gain_map),
                        }
                    }
                }
                // Restart
